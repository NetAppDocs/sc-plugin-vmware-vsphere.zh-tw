...
---
= 瞭解 SnapCenter Plug-in for VMware vSphere
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: 使用 NetApp Backup and Recovery 增強 VMware 保護，該功能提供 3-2-1 備份、勒索軟體防護、自動化備份管理、更高的可擴充性、更低的成本，以及透過單一介面對多個 vCenter 執行個體的資料儲存區和 VM 進行集中式備份管理。您可以將 SCV 原則、快照和排程遷移到 NetApp Backup and Recovery，並從單一主控台管理所有 VM 保護。如需詳細資訊，請參閱link:https://docs.netapp.com/us-en/data-services-backup-recovery/["NetApp Backup and Recovery 文件"]。

====
SnapCenter Plug-in for VMware vSphere 是一個獨立的虛擬設備（Open Virtual Appliance 格式），可為虛擬機和資料存放區提供資料保護服務，並支援 SnapCenter 應用程式型外掛程式的資料保護服務。本文件說明如何部署及使用 SnapCenter Plug-in for VMware vSphere，並包含快速入門資訊。SnapCenter Plug-in for VMware vSphere 以 Linux 為基礎的虛擬設備形式部署。
...
+
如需更多信息，請參閱適用ONTAP tools for VMware vSphere文件。此外，請參閱 https://imt.netapp.com/imt/imt.jsp?components=180121;&solution=1517&isHWU&src=IMT["NetApp 互通性對照表工具"^]有關ONTAP工具支援的版本的最新資訊。
