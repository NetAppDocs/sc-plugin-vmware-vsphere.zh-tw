...
如需更多信息，請參閱適用ONTAP tools for VMware vSphere文件。如需了解支援的版本，請訪問 https://imt.netapp.com/imt/imt.jsp?components=180121;&solution=1517&isHWU&src=IMT["NetApp 互通性對照表工具"^]。

.關於這項工作
* 您可以隨時在資源組中新增或刪除資源。
* 若要備份單一資源（例如虛擬機器），請建立僅包含該資源的資源群組。
* 若要備份多個資源，請建立一個包含所有要保護的資源的資源群組。
* 針對 MetroCluster 環境中的 FlexGroup 磁碟區，如果您使用的是 ONTAP 9.8 或 9.9，請在切換或切回後，於備份資源群組之前，重新啟動 SnapCenter Plug-in for VMware vSphere 服務並重新同步 SnapMirror 關係。在 ONTAP 9.8 中，切回後備份可能會卡住；此問題已在 ONTAP 9.9 中解決。
...
當您嘗試建立資源組時， SnapCenter會執行相容性檢查。總是參考 https://imt.netapp.com/imt/imt.jsp?components=180121;&solution=1517&isHWU&src=IMT["NetApp互通性對照表工具IMT （不含）"^]有關SnapCenter支援的最新資訊。不相容的原因可能包括：
