...
---
= SnapCenter Plug-in for VMware vSphere 的資料保護工作流程
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: 使用 NetApp Backup and Recovery 增強 VMware 保護，該功能提供 3-2-1 備份、勒索軟體防護、自動化備份管理、更高的可擴充性、更低的成本以及對多個 vCenter 執行個體的集中管理。您可以將 SCV 原則、快照和排程遷移到 NetApp Backup and Recovery，並從單一主控台管理所有 VM 保護。如需詳細資訊、請參閱link:https://docs.netapp.com/us-en/data-services-backup-recovery/["NetApp Backup and Recovery 文件"]。
